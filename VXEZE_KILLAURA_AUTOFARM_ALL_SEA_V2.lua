...
-- ================= LEVEL QUEST SYSTEM — ALL SEA =================
-- Danh sách quest theo level. Tên/ID quest có thể thay đổi theo bản game,
-- nên phần chọn quest ưu tiên tìm NPC/quest UI đang có trong map.
getgenv().KA["Auto Quest"] = true

local function GetPlayerLevel()
    local data = J:FindFirstChild("Data")
...
local function FindQuestNPC()
    local candidates = {
        ...
        "Pirate Quest Giver", "Marine Quest Giver", "Desert Adventurer",
        "Snow Quest Giver", "Ice Quest Giver", "Colosseum Quest Giver",
        "Swan Pirate Quest Giver", "Factory Staff", "Area 1 Quest Giver",
        "Area 2 Quest Giver", "Area 3 Quest Giver"
    }
...
local function GetQuestTargetLevel()
    -- Mốc level phổ biến của Blox Fruits; chọn khoảng phù hợp với level hiện tại.
    local lv = GetPlayerLevel()
    local ranges = {
        {1, 9}, {10, 14}, {15, 29}, {30, 59}, {60, 89},
        {90, 119}, {120, 149}, {150, 174}, {175, 189}, {190, 209},
        {210, 249}, {250, 299}, {300, 374}, {375, 449}, {450, 474},
        {475, 524}, {525, 549}, {550, 624}, {625, 649}, {650, 699},
        {700, 724}, {725, 774}, {775, 799}, {800, 849}, {850, 899},
        {900, 949}, {950, 999}, {1000, 1049}, {1050, 1099}, {1100, 1149},
        {1150, 1199}, {1200, 1249}, {1250, 1299}, {1300, 1349},
        {1350, 1399}, {1400, 1449}, {1450, 1499}, {1500, 1549},
        {1550, 1599}, {1600, 1649}, {1650, 1699}, {1700, 1749},
        {1750, 1799}, {1800, 1849}, {1850, 1899}, {1900, 1949},
        {1950, 1999}, {2000, 2049}, {2050, 2099}, {2100, 2149},
        {2150, 2199}, {2200, 2249}, {2250, 2299}, {2300, 2349},
        {2350, 2399}, {2400, 2449}, {2450, 2499}, {2500, 2550},
        {2551, 2600}, {2601, 2650}
    }
...
local function AcceptLevelQuest()
    if not getgenv().KA["Auto Quest"] then return end

    local npc = FindQuestNPC()
    if not npc then return end

    local hrp = npc:FindFirstChild("HumanoidRootPart") or npc:FindFirstChild("Head")
    local myRoot = J.Character and J.Character:FindFirstChild("HumanoidRootPart")
    if not hrp or not myRoot then return end

    -- Chỉ dịch chuyển tới NPC khi chưa có quest đang làm.
    local main = J:FindFirstChild("PlayerGui")
    local questGui = main and main:FindFirstChild("Main")
    local hasQuest = questGui and questGui:FindFirstChild("Quest") ~= nil
    if hasQuest then return end

    myRoot.CFrame = hrp.CFrame + Vector3.new(0, 2, 0)

    -- Hỗ trợ NPC có ClickDetector.
...
getgenv().KA["Above Mob Height"] = 15
